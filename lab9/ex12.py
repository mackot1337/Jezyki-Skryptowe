...
class OutlierDetector(SeriesValidator):
    ...
    def analyze(self, series: TimeSeries) -> list[str]:
        ...
        std: float = float(np.std(valid_values))
    ...
    def analyze(self, series: TimeSeries) -> list[str]:
        ...
        for i, val in enumerate(series.values):
            if val == 0 or val is None:
                ...
                if count >= 3 and block_start is not None:
                    anomalies.append(
    ...
    def loadDataForStations(self, meta: dict[str, Any], targetStations: Optional[list[str]]) -> None:
        ...
        with open(meta["path"], "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                rawDate: str = row[0].strip()
                dt: Union[datetime.datetime, datetime.date]
                try:
                    dt = datetime.datetime.strptime(rawDate, "%m/%d/%y %H:%M")
                except ValueError:
                    raise ValueError(f"Nie można sparsować daty: {rawDate}")
